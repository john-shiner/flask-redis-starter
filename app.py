...
def index():
    ...
    return render_template("counter.html", hit_counts=redis.get('hits'), title=TITLE, desc=DESC, status=status)
...
def hello():
    ...
    return render_template('hello.html', form=form, name=session.get('name'), title=TITLE, desc=DESC, status=status)
